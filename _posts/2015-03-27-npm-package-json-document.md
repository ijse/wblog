...
<ul>
 	<li><code>"scripts": { "start": "node server.js" }</code>
如果在项目根目录下含有<code>server.js</code>文件，则NPM会自动设置此值。</li>
 	<li><code>"scripts": { "preinstall": "node-gyp rebuild" }</code>
如果在项目根目录下含有<code>binding.gyp</code>文件，则NPM会自动设置此值。</li>
 	<li><code>"contributors": [...]</code>
如果项目根目录下含有<code>AUTHORS</code>文件，则NPM会自动将每一行以<code>Name &lt;i@ijser.cn&gt; (url)</code>的格式读取并设定此字段。</li>
</ul>